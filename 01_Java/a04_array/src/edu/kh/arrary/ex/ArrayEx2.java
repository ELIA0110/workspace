...
//		String input = sc.next(); // 문자열 (단어) 입력받기
		
		// sc.nextChar() 은 존재하지 않으므로
		// sc.next()를 이용해 문자열을 입력 받은 후
		// String.charAt(index)를 이용해 0번 인덱스 값은 반환 받기
		...
		for(int row=0 ; row < arr.length ; row++) {
			...
			for(int col=0 ; col < arr[row].length ; col++) {
				...
				if(arr[row][col] == input) {
					...
					// %c : char
					System.out.printf("%c는 %d행 %d열에 있습니다.",
							arr[row][col], row, col);
	...
	// hint : sum, 2둥 for문을 이용한 2차원 배열요소 모두 접근
...
//			}
//		}
//		int avg = sum / count; // count == arr.length * arr[0].length 
