...
public class ExceptionService {
	...
	// 어디서든 접근(호출) 가능한 반환형이 없고 매개변수(전달 받을 값이) 없는 메서드
	public void ex2() {
...
//		
	...
	public void ex3() {
		...
		try {
			...
			// 부모 타입 예외 참조 변수로 
			// 자식 타입 예외 객체를 참조한다.
			// --> 부모 타입으로 모든 자식 예외를 잡아서 처리할 수 있다.
	...
	public void ex7() {
		// throws : 메서드에서 발생한 예외를 호출한 상위 메서드로 던지는 것(전달)
		//			(책임 전가)
...
//		throw new EOFException(); // 자식 타입 예외 객체
		// EoFException == IOEsception의 자식
		
		throw new DataFormatException();
		// IOException과 관련 없는 예외
		// -> 별도 처리 필요 -> IOException의 자식
	}
