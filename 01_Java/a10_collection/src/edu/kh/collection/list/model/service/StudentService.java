...
public class StudentService {
	...
	public StudentService() {
		...
		studentList.add(new Student(2,2,30,"청명", 'M',30));
		studentList.add(new Student(1,1,13,"조걸", 'M',50));
		studentList.add(new Student(8,8,2,"유이설", 'F',13));
		studentList.add(new Student(5,2,18,"김동룡", 'M',42));
	}
	...
	public boolean addStudent(int grade, int ban, int number, String name, char gender, int score) {
		...
		// 1) 중복 검사 메서드 호출(중복이면 true, 아니면 flase)
	...
	public List<Student> selectGender(char gender) {
		...
		// -> 매 반복마다 배열 또는 컬렉션의 요소를 순서대로 하나씩 꺼내어 
		//    참조 변수에 대입하는 반복문
	...
	public List<Student> sortScore() {
		...
		// -> 이 기능을 사용하려면 List의 요소(Student)에 정렬 기준을 정하는 코드가 있어야한다.
		
		Collections.sort(studentList); 
		// -> 점수 오름차순(점점 커지는 순서) 정렬
		
		Collections.reverse(studentList);
		// -> List 순서를 뒤집음(오름차순 -> 내림차순)
		return studentList;
