...
// Client(고객) : 서버가 제공하는 서비스를 요청해서 제공받는 컴퓨터

public class Client {

	public void start() {
		
		// 1. 서버의 IP주소와 서버가 정한 포트 번호를 이용해서
		//    서버와 연결하는 소켓 객체 생성
		
		String ip ="192.168.10.28"; // loop back ip(현재 컴퓨터 나타내는 IP)
		...
		try {
			...
			if(socket != null) { // 서버와 연결이 성공한 경우
				...
				String serverMessage = br.readLine(); // 한 줄 읽기
				System.out.println(serverMessage);		
				// 2023-08-17 10:15:29[서버 접속 성공]
				
				
				// 4-2) 클라이언트 -> 서버로 입력 받은 메시지 전달
