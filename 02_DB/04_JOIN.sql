...
-- 두 컬름을 이용해서 EMPLOYEE, DEPARTMENT 테이블을 하나 합침
...
WHERE DEPT_CODE = DEPT_ID
	AND DEPT_TITLE = '해외영업2부' -- 오라클 방식 조인시 추가 조건은 AND
ORDER BY EMP_ID;
...
FROM EMPLOYEE, JOB -- 테이블명 별칭 순서로 작성
...
ORDER BY DEPT_CODE DESC, JOB_NAME;
...
SELECT EMP_NAME, JOB_NAME, SALARY, 
	CASE 
		WHEN BONUS > 0 THEN SALARY *12 *(1+BONUS) 
...
FROM EMPLOYEE A
LEFT JOIN EMPLOYEE B ON (A.DEPT_CODE  = B.DEPT_CODE )
ORDER BY 1;
