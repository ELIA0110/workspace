...
SELECT EMP_ID ,SUBSTR(EMAIL, 1, INSTR(EMAIL,'@')-1) "아이디" 
FROM EMPLOYEE
ORDER BY "아이디";
...
SELECT SYSDATE, ADD_MONTHS(SYSDATE, 1)  "한달 후" FROM DUAL;
 ...
 * 숫자 -> 문자열
 * 포맷 
 * 1) 9 : 숫자 한 칸을 의미, 오른쪽 정렬 (빈 칸은 띄어쓰기)
...
-- 포맷의 역할 : 문자열 또는 숫자의 패턴이 날짜의 어떤 부분을 가리키고 있는지 나타냄
...
/*******연도 패턴 Y,R 차이점*******/
-- 연도 표기 시 두자리만 작성되어있는 경우
-- 50 미만 : Y,R 둘 다 누락된 연도 앞부분에 현재 세기(2000년대) 추가
-- 50 이상 : Y는 현재 세기가 추가, R은 이전 세기가 추가
...
-- 단, 인센티브가 없으면 0으로 표시
-- 연봉 내림차순으로 조회
SELECT EMP_NAME 이름,SALARY * 12 연봉, NVL(SALARY * 12 * BONUS, 0) 인센티브
FROM EMPLOYEE
ORDER BY 2 DESC;
...
-- DECODE(컬럼명 | 계산식, 조건1, 결과1, 조건2, 결과2...[, 아무것 만족 X])

-- 컬럼명 | 계산식의 값이 일치하는 조건이 있으면
-- 해당 조건 오른쪽에 작성된 결과가 반환된다.

-- 모든 조건 불만족 시 [,아무것 만족 X]가 반환됨.
...
-- 모든 사원 중
-- 가장 빠른 입사일, 최근 입사일
-- 이름 오름 차순에서 제일 먼저 작성되는 이름, 마지막에 작성되는 이름
...
-- COUNT(DISTINCT 컬럼명) : 
	...
	-- EX) A A B C D D D E : 5개(중복은 한번만 카운트)
