...
TRUNC(MONTHS_BETWEEN(TRUNC(SYSDATE), TO_DATE(SUBSTR(PROFESSOR_SSN,1,6),'RRMMDD')) / 12)
FROM TB_PROFESSOR
...
FROM TB_STUDENT 
WHERE TRUNC((ENTRANCE_DATE - 
 	  TO_DATE(SUBSTR(STUDENT_SSN,1,6),'RRMMDD')) / 365) = 19; 
...
FROM TB_STUDENT
WHERE STUDENT_NAME IN (SELECT STUDENT_NAME FROM TB_STUDENT
