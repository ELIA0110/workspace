...
SELECT EMP_NAME, DEPT_CODE
FROM EMPLOYEE e 
WHERE DEPT_CODE = (SELECT DEPT_CODE FROM EMPLOYEE e 
					WHERE EMP_NAME = '노옹철');
...
WHERE SALARY IN (SELECT MAX(SALARY)
				FROM EMPLOYEE e 
				GROUP BY DEPT_CODE);
-- IN 괄호 안에 SELECT 결과 7행이 들어가게 된다
...
NATURAL JOIN JOB j 
WHERE JOB_NAME = '과장'
AND SALARY > ALL (SELECT SALARY FROM EMPLOYEE e2
					NATURAL JOIN JOB j2 WHERE JOB_NAME = '차장');
--AND SALARY > (SELECT MAX(SALARY) FROM EMPLOYEE e2
--					NATURAL JOIN JOB j2 WHERE JOB_NAME = '차장');
...
AND NOT( EXTRACT(YEAR FROM HIRE_DATE) = '2000');
...
WHERE ROWNUM BETWEEN 1 AND 10; --> 1부터 포함해야 된다!
...
-- 사번, 사원명, 전화번호, 고용일, 부서명
SELECT EMP_ID, EMP_NAME, TO_CHAR(HIRE_DATE, 'YY/MM/DD'), DEPT_TITLE 
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
