...
SELECT DEPT_CODE /*묶인 그룹의 이름*/, SUM(SALARY) /*그룹별 급여 합*/
FROM EMPLOYEE
GROUP BY DEPT_CODE -- EMPLOYEE 테이블에서
					-- DEPT_CODE가 같은 행끼리 그룹으로 묶어라
ORDER BY DEPT_CODE; -- 조회 결과(RESULT SET)을 DEPT_CODE 오름차순 정렬
...
FROM EMPLOYEE
-- WHERE SALARY >= 3000000 : 급여가 300만 이상인 사원(행)만 조회 결과에 포함
							--> 그룹 조건에 맞지 않음
GROUP BY DEPT_CODE
HAVING AVG(SALARY) >= 3000000 -- 묶여진 그룹의 급여 평균이 300만 이상인 경우
							  -- 조회 결과에 포함(D6,D9)
ORDER BY DEPT_CODE;


-- 부서별 그룹의 급여 합계 중 9백만원을 초과하는 
-- 부서의 부서코드와 급여 합계 조회
-- 부서 코드 순으로 정렬
SELECT DEPT_CODE , SUM(SALARY)
FROM EMPLOYEE
