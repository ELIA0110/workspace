...
SELECT EMP_NAME, HIRE_DATE, 
	FLOOR((SYSDATE - HIRE_DATE)/365) FROM EMPLOYEE; 
...
SELECT EMP_ID 사원, EMP_ID 이름, DEPT_CODE 부서코드, PHONE 전화번호
FROM EMPLOYEE
WHERE DEPT_CODE <> 'D9';
...
--> 11행이 조회되야 하는데 9행만 조회되었다!
-- 왜? DEPT_CODE가 NULL행이 2행이 있어서 그렇다!
...
-- EMPLOYEE 테이블에서
-- EMAIL의 아이디 중 '_' 앞 쪽 글자의 수가 3글자인 사원의
-- 사번, 이름, EMAIL 조회
SELECT EMP_ID, EMP_NAME , EMAIL 
FROM EMPLOYEE
WHERE EMAIL LIKE '____%';
--> 문제점 : 와일드 카드 표기법 '_'와 기준으로 삼을 문자열 '_'가 둘 다 
--			와일드 카드로 해석되어 조건식의 의미가 의도와 다르게 작성됨.
...
-- 날짜 형식의 문자열이기 때문에
-- HIRE_DATE(DATE 타입) 비교시 자동으로 형변환(Parsing)이 진행된다.
...
SELECT EMP_ID 사원, EMP_NAME 이름,SALARY *12 연봉
FROM EMPLOYEE
WHERE SALARY *12 >= 35000000
ORDER BY 연봉; -- SELECT절 해석 후  ORDER BY절이 해석되기 때문에
...
SELECT EMP_NAME, PHONE
FROM EMPLOYEE
ORDER BY PHONE DESC NULLS LAST; --> 내림차순 + NULL은 마지막에 배치
...
-- 이름, 부서코드, 직급코드를 
-- 부서코드 내림차순, 직급코드 오름차순, 이름 오름차순으로 조회
